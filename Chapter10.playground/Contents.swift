...
/*
 ...
 애플의 프레임워크는 키-값 코딩 등 많은 곳에 키 경로를 활용하므로, 애플 프레임워크 기반의 애플리케이션을 만든다면 잘 알아두기 바랍니다.
 자기 자신을 나타내는 키 경로인 \.self를 사용하면 인스턴스 그 자체를 표현하는 키 경로가 된다.
 또, 컴파일러가 타입을 유추할 수 있는 경우에는 키경로에서 타입 이름을 생략할 수도 있다.
 
 스위프트 5.2 버전부터 (SomeType) -> Value 타입의 클로저를 키 경로 표현으로 대체하여 사용할 수 있다.
 예제 10-17 클로저를 대체할 수 있는 키 경로 표현은 클로저 공부 후 공부할 예정.
 */
